    ...
    static List<String> lines(String section) {
        return switch (section.toLowerCase(Locale.ROOT)) {
            ...
            case "governments", "government", "nation", "nations", "state", "states", "city", "cities", "members", "officers", "invitations" -> List.of(
                    ...
                    "Kicking an ancestor official's local membership requires a higher-ranking caller; their unaffected ancestor citizenship/office is preserved.",
                    ...
                    "<kind> setting <government> <key> <value|inherit> — executive policy; requireLegislationForPolicy restricts national mechanical policies only, not local policies or open membership.",
            ...
            case "chunks", "chunk", "claims", "map" -> List.of(
                    "chunk claim <nation> <chunks> (alias: claim) — national officials only; state/city references cannot claim. Omitting nation uses your nation.",
                    "chunk unclaim <nation> <chunks> (alias: unclaim) — national authority plus title authority; cannot split the nation's connected territory.",
                    "chunks is here or comma-separated dimension|x|z keys in your current dimension, maximum 64. Duplicates are rejected; each batch succeeds or fails as a whole.",
                    "chunk assignstate <nation> <chunks> <state_or_none> — national officials allocate existing land to a state of that nation. Changing state clears incompatible city allocations.",
                    "chunk assigncity <state> <chunks> <city_or_none> — state or national officials allocate that state's chunks to one of its cities.",
                    "none clears an allocation. Clearing a state also clears its city. Repeating the same state preserves its valid city assignment.",
                    "chunk autoclaim <on|off> — moving claims for your nation; national authority is rechecked on every movement.",
                    ...
                    "New claims belong to a nation, have no state/city assignment, and have a public nation treasury title. Claim fees are paid by the actor to that nation.",
                    "Allocations have no claim fee and need not be adjacent. Public title and future sale proceeds move to the most-specific assigned government; no treasury balances move.",
                    "Private player/company titles, improvements and permits survive allocations. Changing a public title clears its old permits. Buying property never changes jurisdiction.",
                    "maxClaimsPerNation bounds national claims; maxClaimsPerCity now bounds city allocations. Existing over-limit territory is preserved, not removed.",
                    ...
                    "Claims pledged to contracts, treaties, economy property listings or loans cannot be unclaimed, retitled or reallocated.",
                    "A city/state must have no allocated chunks before deletion, even with cascade. National cascade can remove only its unencumbered public claims; private titles block it.");
            ...
            case "diplomacy" -> List.of(
                    ...
                    "diplomacy peace <fromNation> <toNation> <offerAmount> <demandAmount> <chunk=destinationGovernment,...|-> [message] — propose immutable peace terms.",
                    ...
                    "Treaties transfer existing public claims between nations, never create claims or confiscate private property. Nation destinations leave state/city unassigned; state/city destinations explicitly negotiate those allocations.",
                    "National connectivity/claim limits and city allocation limits apply; city allocations need not be connected. Selected claims remain reserved until conclusion/expiry.",
                    ...
                    "Malformed active treaty references require operator repair. Settlement reports the blocked field before transferring money or land.",
            ...
            case "contracts", "contract" -> List.of(
                    ...
                    "contract create <government> <title> <description> <chunkKey,...|here> — official selects public chunks in that nation's territory or explicit state/city allocation; unassigned national/state claims are eligible.",
                    ...
                    "Chunk-count limits apply to new selections, not unchanged existing awards; title, reservation, accepted-bid and escrow checks still apply.",
            ...
            case "admin" -> List.of(
                    ...
                    "admin reassign <chunkKey|here> <government> — explicit nation/state/city reassignment, ignoring adjacency; public title follows the destination and private titles/permits are preserved.",
                    ...
                    "Repair clears dangling memberships/stale officers/invites and financially inert empty orphan governments. It never guesses national claim ownership, invents shares, releases stock reservations or discards escrow.",
                    "All orphan claims and ambiguous leaders/IDs remain for explicit reassignment/appointment. Settle financial obligations through the economy before deleting anything.",
