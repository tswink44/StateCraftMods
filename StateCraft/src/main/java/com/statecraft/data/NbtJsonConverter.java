 ...
 * <p>
 * NBT types are preserved using a type-annotation scheme so the JSON is
 * both human-readable and losslessly round-trippable.
 * <p>
 * Encoding rules:
 * <ul>
 *   <li>CompoundTag  → JsonObject  (keys are strings, values are recursively encoded)</li>
 *   <li>ListTag       → JsonArray   (elements are recursively encoded)</li>
 *   <li>String         → plain JSON string</li>
 *   <li>Int / Long / Short / Byte / Float / Double → JSON number</li>
 *   <li>ByteArray / IntArray / LongArray → JsonArray of numbers</li>
 * </ul>
 * To distinguish numeric types on deserialization, non-string leaf values
 * are wrapped: {@code {"_nbtType":"long","_value":42}}
 */
public final class NbtJsonConverter {
    ...
    private static JsonElement tagToJson(Tag nbt) {
        return switch (nbt.getId()) {
            ...
            default -> new JsonPrimitive(nbt.getAsString()); // fallback
        };
    }
    ...
    /**
     * Convert a JsonObject back to a CompoundTag.
     */
    ...
    private static Tag jsonToTag(JsonElement element) {
        ...
        if (element.isJsonPrimitive()) {
            ...
            // Untyped numbers default to int (shouldn't happen in round-trip)
            if (prim.isNumber()) {
                return IntTag.valueOf(prim.getAsInt());
            }
            if (prim.isBoolean()) {
